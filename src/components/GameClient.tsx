...
import { AdBanner } from "@/components/AdSenseScript";
...
export default function GameClient() {
  ...
  const handleGuess = useCallback(async (word: string) => {
    ...
    const prevScore = attempts.length > 0 ? attempts[0].score : null; // first = highest after sort
    const direction = getDirection(score, prevScore);
    const isWin = score === 100;

    const newAttempts = [...attempts, { word, score, direction }].sort((a, b) => b.score - a.score);
    setAttempts(newAttempts);

    if (isWin) {
      setWon(true);
      const newSolved: SolvedWord = { word: daily.word, theme: daily.theme, attempts: newAttempts.length, revealed: false, date: Date.now() };
  ...
  const handleReveal = useCallback(() => {
    ...
    setShowRevealConfirm(false);
    setRevealed(true);
    const newSolved: SolvedWord = { word: daily.word, theme: daily.theme, attempts: attempts.length, revealed: true, date: Date.now() };
    const newSolvedWords = [newSolved, ...solvedWords];
    setSolvedWords(newSolvedWords);
  ...
  const handleNextWord = useCallback(() => { generateNew(solvedWords); }, [generateNew, solvedWords]);

  const themeEmoji = daily ? (themeEmojis[daily.theme] ?? "🔤") : "🔤";
  const bestScore = attempts.length > 0 ? Math.max(...attempts.map((a) => a.score)) : 0;
  const blocked = won || revealed;
  const showAdAfterAttempts = attempts.length >= 3;

  if (generatingWord) {
    return (
      <div className="min-h-screen bg-[#0d0820] flex flex-col items-center justify-center gap-4" role="status" aria-label="Cargando">
  ...
  return (
    <>
      <div className="min-h-screen bg-[#0d0820] text-white flex flex-col">
        {/* Background */}
        ...
        {/* Top banner ad – visible after some interaction */}
        {showAdAfterAttempts && (
        ...
        <div className="relative z-10 flex flex-1">
          {/* Sidebar desktop */}
          <aside className="hidden lg:block w-72 flex-shrink-0" aria-label="Historial de partidas">
          ...
          {/* Game area */}
          <main className="flex-1 max-w-lg mx-auto px-4 py-6 pb-4 w-full" id="main-content">
            ...
            <header className="text-center mb-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  ...
                  aria-label="Ver historial de palabras resueltas"
                >
                  🏆
                  {solvedWords.length > 0 && (
                    <span className="absolute -top-1 -right-1 w-4 h-4 bg-violet-500 rounded-full text-[10px] font-bold flex items-center justify-center" aria-label={`${solvedWords.length} palabras`}>
                      {solvedWords.length}
                    </span>
                  )}
                </button>
              ...
              <p className="text-white/30 text-xs mt-1 hidden sm:block">
                Juego semántico con IA · Escribe palabras y descubre la secreta
              </p>
            </header>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-4 mb-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-lg font-bold text-white flex items-center gap-2" aria-label={`Tema: ${daily?.theme}`}>
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  ...
                  <div className="text-2xl font-bold text-white" aria-label={`${attempts.length} intentos`}>{attempts.length}</div>
                </div>
                <button
                  onClick={() => generateNew()}
            ...
            {attempts.length > 0 && (
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4 mb-4">
                ...
                <div className="w-full h-3 bg-white/10 rounded-full overflow-hidden" role="progressbar" aria-valuenow={bestScore} aria-valuemin={0} aria-valuemax={100} aria-label="Progreso">
                  <div
                ...
                {bestScore >= 70 && !blocked && (
                  <p className="text-xs text-violet-300 mt-1.5 text-center animate-pulse" role="alert">¡Estás muy cerca! 🔥</p>
                )}
              </div>
            )}
            ...
            {revealed && daily && (
              <div className="bg-amber-500/15 border border-amber-400/30 rounded-2xl p-4 mb-4 text-center" role="alert">
            ...
            {!blocked && (
              <div className="mb-4 space-y-2">
                ...
                {scoringLoading && (
                  <div className="flex items-center gap-2 mt-2 text-xs text-violet-300 animate-pulse" role="status" aria-live="polite">
                    <div className="w-3 h-3 rounded-full border border-violet-400 border-t-transparent animate-spin" aria-hidden="true" />
                ...
                {!showRevealConfirm ? (
                  ...
                  <div className="bg-amber-500/10 border border-amber-400/20 rounded-xl p-3 text-center" role="dialog" aria-label="Confirmar revelación">
                    <p className="text-amber-200 text-xs mb-2">¿Seguro que quieres revelar la palabra?</p>
                    <div className="flex gap-2 justify-center">
                      <button onClick={handleReveal} className="bg-amber-500 hover:bg-amber-400 text-black text-xs font-bold px-4 py-1.5 rounded-lg transition-all">Sí, revelar</button>
                      <button onClick={() => setShowRevealConfirm(false)} className="bg-white/10 hover:bg-white/15 text-white/60 text-xs px-4 py-1.5 rounded-lg transition-all">Cancelar</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            {/* Mid-game ad – apareix a partir de 5 intents */}
            {attempts.length >= 5 && !blocked && (
              <div className="mt-4">
            ...
            {/* How to play – SEO text block */}
            {attempts.length === 0 && (
              <section className="mt-8 bg-white/3 border border-white/8 rounded-2xl p-5" aria-label="Cómo jugar">
                <h2 className="text-sm font-bold text-white/60 uppercase tracking-wider mb-3">¿Cómo se juega?</h2>
                <ol className="space-y-2 text-white/40 text-xs leading-relaxed list-none">
                  {[
                    ["🎯","Escribe cualquier palabra en español en el campo de arriba."],
                    ["🤖","La IA analiza la similitud semántica con la palabra secreta."],
                    ["📊","Recibes una puntuación del 0% (lejos) al 100% (¡es la palabra!)."],
                    ["🔥","Los intentos se ordenan: el más cercano siempre arriba."],
                    ["🏆","¡Adivina la palabra con el menor número de intentos posible!"],
                  ].map(([icon, text], i) => (
          ...
          <aside className="hidden xl:block w-40 flex-shrink-0 p-4" aria-label="Publicidad">
            <div className="sticky top-4">
              <AdBanner slot="5566778899" format="vertical" />
            </div>
          </aside>
        </div>
        ...
        {blocked && (
          <div className="relative z-10 w-full px-4 py-2">
            <AdBanner slot="9988776655" format="horizontal" className="max-w-2xl mx-auto" />
          </div>
        )}

        {/* Footer */}
        <footer className="relative z-10 border-t border-white/8 mt-4 py-4 px-4">
          <div className="max-w-2xl mx-auto">
            <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-white/25">
              <p className="text-white/20">
                <strong className="text-white/35">Adivina la Palabra</strong> — Juego semántico con IA en español
              </p>
              <nav className="flex gap-4" aria-label="Navegación del pie">
            ...
            <p className="text-white/15 text-xs mt-2 max-w-xl">
              Juego de palabras gratuito en español. Adivina la palabra secreta usando pistas de
              similitud semántica generadas por inteligencia artificial. Sin registro, sin límite de partidas.
            </p>
          </div>
        </footer>
      </div>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="Historial">
          ...
          <div className="absolute right-0 top-0 h-full w-80 bg-[#120d25] border-l border-white/10 overflow-y-auto p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setSidebarOpen(false)} className="text-white/50 hover:text-white text-xl" aria-label="Cerrar">✕</button>
            </div>
            <SolvedWordsSidebar solvedWords={solvedWords} />
          </div>
        </div>
